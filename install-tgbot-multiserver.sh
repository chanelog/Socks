#!/bin/bash
# ============================================================
#   OGH-ZIV Multi-Server Installer
#   Support: VPS Indonesia (Master + Bot) & VPS Singapore (Worker)
...
echo -e "${C}  ╠══════════════════════════════════════════════════════╣${N}"
echo -e "${C}  ║${N}  Support: VPS Indonesia (Bot) & Singapore (Worker)  ${C}║${N}"
echo -e "${C}  ╚══════════════════════════════════════════════════════╝${N}"
echo ""
...
echo ""
echo -e "  ${W}[1]${N} 🇮🇩  VPS Indonesia  ${DIM}— Install Bot Telegram (Master)${N}"
echo -e "  ${W}[2]${N} 🇸🇬  VPS Singapore  ${DIM}— Install API Worker (Slave)${N}"
echo -e "  ${W}[0]${N}     Keluar"
echo ""
...
# ════════════════════════════════════════════════════════════
#   MODE 1 — VPS INDONESIA (Master Bot)
# ════════════════════════════════════════════════════════════
install_indo() {
  echo -e "${C}  ╔══════════════════════════════════════════════════════╗${N}"
  echo -e "${C}  ║   🇮🇩  INSTALL BOT TELEGRAM — VPS INDONESIA          ║${N}"
  echo -e "${C}  ╚══════════════════════════════════════════════════════╝${N}"
  echo ""
  ...
  # ── Setup Server SG (opsional) ─────────────────────────────
  echo ""
  echo -e "${C}  ════════════════════════════════════════════════════${N}"
  echo -e "${C}  🌍  KONFIGURASI SERVER SG (Opsional)${N}"
  echo -e "${C}  ════════════════════════════════════════════════════${N}"
  echo -e "  ${DIM}Kosongkan jika belum ada VPS SG. Bisa diisi nanti via bot.${N}"
  echo ""

  echo -ne "  ${C}IP/Domain VPS SG${N} (kosong=skip): "
  read -r inp_sg_host
  ...
  if [[ -n "$inp_sg_host" ]]; then
    echo -ne "  ${C}Port ZIVPN di VPS SG${N} [5667]: "
    read -r inp_sg_port
    [[ -z "$inp_sg_port" ]] && inp_sg_port="5667"

    echo -ne "  ${C}API URL VPS SG${N} (contoh: http://${inp_sg_host}:8765): "
    read -r inp_sg_api_url
    [[ -z "$inp_sg_api_url" ]] && inp_sg_api_url="http://${inp_sg_host}:8765"

    echo -ne "  ${C}API Key${N} (buat sendiri, harus sama di VPS SG): "
    read -r inp_sg_api_key
    [[ -z "$inp_sg_api_key" ]] && inp_sg_api_key="ogh-ziv-$(openssl rand -hex 8)"
    echo -e "  ${G}  API Key: ${W}${inp_sg_api_key}${N}"
  fi
...
EOF
  ...
  # ── Simpan konfigurasi servers.json ───────────────────────
  local SG_ENABLED="false"
  [[ -n "$inp_sg_host" ]] && SG_ENABLED="true"
  ...
  cat > "$SERVERS_JSON" <<EOF
{
  "indo": {
    "label":   "🇮🇩 Indonesia",
    "enabled": true,
    "host":    "$(curl -s4 --max-time 5 ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}')",
    "port":    "$(python3 -c "import json;d=json.load(open('/etc/zivpn/config.json'));print(d.get('listen',':5667').lstrip(':'))" 2>/dev/null || echo 5667)",
    "api_url": "",
    "api_key": "",
  ...
  "sg": {
    ...
    "host":    "${inp_sg_host}",
    "port":    "${inp_sg_port}",
    "api_url": "${inp_sg_api_url}",
    "api_key": "${inp_sg_api_key}",
    "note":    "Server Singapore — Remote",
    "stock":   -1
  }
}
EOF
...
EOF
  ...
  echo ""
  echo -e "${C}  ╔══════════════════════════════════════════════════════╗${N}"
  echo -e "${C}  ║   ✦  INSTALASI VPS INDONESIA SELESAI!               ║${N}"
  echo -e "${C}  ╠══════════════════════════════════════════════════════╣${N}"
  ...
  if [[ -n "$inp_sg_host" ]]; then
    printf "  ${C}║${N}  %-20s : ${W}%s${N}\n" "VPS SG Host" "${inp_sg_host}"
    printf "  ${C}║${N}  %-20s : ${W}%s${N}\n" "API Key SG" "${inp_sg_api_key}"
  fi
  echo -e "${C}  ╠══════════════════════════════════════════════════════╣${N}"
  ...
  if [[ -n "$inp_sg_host" ]]; then
    echo -e "${Y}  ⚠️  LANGKAH SELANJUTNYA — VPS SG:${N}"
    echo -e "  Jalankan perintah ini di ${W}VPS Singapore${N}:"
    echo ""
    echo -e "  ${W}bash <(curl -Ls https://raw.githubusercontent.com/chanelog/Socks/main/install-tgbot-multiserver.sh)${N}"
    echo ""
    echo -e "  Pilih ${W}[2] VPS Singapore${N}, lalu masukkan:"
    echo -e "  • API Key  : ${W}${inp_sg_api_key}${N}"
    echo -e "  • API Port : ${W}8765${N}"
    echo ""
  fi
...
# ════════════════════════════════════════════════════════════
#   MODE 2 — VPS SINGAPORE (Worker)
# ════════════════════════════════════════════════════════════
install_sg() {
  echo -e "${C}  ╔══════════════════════════════════════════════════════╗${N}"
  echo -e "${C}  ║   🇸🇬  INSTALL API WORKER — VPS SINGAPORE            ║${N}"
  echo -e "${C}  ╚══════════════════════════════════════════════════════╝${N}"
  echo ""
  ...
  echo -e "${C}  ════════════════════════════════════════════════════${N}"
  echo -e "  ${DIM}API Key harus sama dengan yang di VPS Indonesia!${N}"
  echo ""

  # Load config lama jika ada
  [[ -f "$WORKER_CONF" ]] && source "$WORKER_CONF" 2>/dev/null

  echo -ne "  ${C}API Key${N} (dari VPS Indonesia) [${API_KEY:--}]: "
  read -r inp_api_key
...
EOF
  ...
  # ── Patch API Key dan Port ke script ─────────────────────
  ...
  cat > "$WORKER_SVC" <<EOF
[Unit]
Description=OGH-ZIV API Worker (Singapore)
After=network.target
Wants=network-online.target
...
EOF
  ...
  echo ""
  echo -e "${C}  ╔══════════════════════════════════════════════════════╗${N}"
  echo -e "${C}  ║   ✦  INSTALASI VPS SINGAPORE SELESAI!               ║${N}"
  echo -e "${C}  ╠══════════════════════════════════════════════════════╣${N}"
  printf  "  ${C}║${N}  %-20s : ${W}%s${N}\n" "IP Publik SG" "${MY_IP}"
  printf  "  ${C}║${N}  %-20s : ${W}%s${N}\n" "API Port" "${inp_api_port}"
  printf  "  ${C}║${N}  %-20s : ${W}%s${N}\n" "API Key" "${inp_api_key}"
  printf  "  ${C}║${N}  %-20s : ${W}http://%s:%s${N}\n" "API URL Lengkap" "${MY_IP}" "${inp_api_port}"
  echo -e "${C}  ╠══════════════════════════════════════════════════════╣${N}"
  ...
  echo -e "${C}  ╚══════════════════════════════════════════════════════╝${N}"
  echo ""
  echo -e "${Y}  ⚠️  PASTIKAN DI BOT TELEGRAM (VPS Indo):${N}"
  echo -e "  Admin Panel → Kelola Server Indo/SG → Edit 🇸🇬 Singapore"
  echo -e "  • Set Host/IP  : ${W}${MY_IP}${N}"
  echo -e "  • Set API URL  : ${W}http://${MY_IP}:${inp_api_port}${N}"
  echo -e "  • Set API Key  : ${W}${inp_api_key}${N}"
  echo -e "  • Aktifkan server SG ✅"
  echo ""
  echo -e "  ${G}✔  VPS Singapore siap menerima perintah dari Bot!${N}"
  echo ""
}
