...
def list_accounts():
    ...
    for line in Path(USERS_DB).read_text().splitlines():
        ...
        if len(parts) >= 3:
            ...
            try:
                if len(exp_raw) > 10:
                    # Format datetime lengkap: "YYYY-MM-DD HH:MM" (akun trial)
                    exp_dt = datetime.strptime(exp_raw, "%Y-%m-%d %H:%M")
                else:
                    # Format tanggal saja: "YYYY-MM-DD" (akun berbayar) — expire akhir hari
                    exp_dt = datetime.strptime(exp_raw, "%Y-%m-%d").replace(hour=23, minute=59, second=59)
                status = "aktif" if exp_dt > now_dt else "expired"
            except ValueError:
                status = "aktif"  # fallback jika format tidak dikenal
            result.append({
                "username": parts[0],
                "exp":      exp_raw,
                "status":   status
