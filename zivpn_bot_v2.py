...
# Struktur default server — edit via bot atau langsung di file
DEFAULT_SERVERS = {
    "indo": {
        "label":   "🇮🇩 Indonesia",
        "enabled": True,
        "host":    "",          # IP/domain VPS Indo — diisi otomatis dari VPS Indo
        "port":    "5667",
        "api_url": "",          # URL API VPS Indo  (kosongkan jika bot jalan di VPS Indo)
        "api_key": "",          # API key untuk auth antar VPS
        "note":    "Server Indonesia",
        "stock":   -1,          # -1 = unlimited, 0 = habis, >0 = stok tersedia
    },
    "sg": {
        "label":   "🇸🇬 Singapore",
        "enabled": False,       # Diaktifkan setelah diisi
        "host":    "",          # IP/domain VPS SG
        "port":    "5667",
        "api_url": "",          # URL API VPS SG  contoh: http://1.2.3.4:8765
        "api_key": "",
        "note":    "Server Singapore",
        "stock":   -1,
    },
}
...
def load_servers() -> dict:
    ...
    if p.exists():
        try:
            ...
            for srv_id, defaults in DEFAULT_SERVERS.items():
                if srv_id not in data:
                    data[srv_id] = defaults
                else:
...
async def cb_admin_kelola_server(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    srvs = load_servers()
    lines = ["🌍 <b>Kelola Server Indo & SG</b>\n━━━━━━━━━━━━━━━━━━━━━━━"]
    for srv_id, srv in srvs.items():
        ...
        api   = "Lokal" if not srv.get("api_url") else "Remote"
        lines.append(
            f"\n{icon} <b>{label}</b>\n"
            f"   Host    : <code>{host}:{port}</code>\n"
            f"   Status  : {enabled_text}  |  Mode: {api}\n"
            f"   Stok    : {stock}"
        )
    ...
    for srv_id, srv in srvs.items():
        label = srv.get("label", srv_id.upper())
        keyboard.append([InlineKeyboardButton(
            f"⚙️ Edit {label}", callback_data=f"admin_srv_edit_{srv_id}"
        )])
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(cb_srv_paket,            pattern="^srv_"))
