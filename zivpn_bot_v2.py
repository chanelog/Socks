...
def create_account(username: str, password: str, days: int, kuota: int,
    ...
    # exp_override: opsional, datetime string "YYYY-MM-DD HH:MM" untuk trial (agar tepat 60 menit)
    if exp_override:
        exp = exp_override
    else:
...
def format_paket_list() -> str:
    ...
    lines = [
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━",
        f"🎁  <b>Trial Gratis</b> — 60 Menit | 1 device",
        "━━━━━━━━━━━━━━━━━━━━━━━",
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🎁 Trial Gratis 60 Menit", callback_data="trial")],
...
async def cb_trial_srv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if is_local_server(srv_id):
            ...
            result = api_call_remote(srv_id, "create_account", {
                ...
                "password":     password,
                "days":         1,
                "exp_override": exp_str,   # ← kirim datetime exact "YYYY-MM-DD HH:MM"
                "kuota":        1,
                "maxlogin":     1,
    ...
    # ── PERBAIKAN: Jadwalkan auto-delete akun trial tepat setelah 60 menit ──
    ...
    if is_local_server(srv_id):
        ...
        if result.get("ok"):
            ...
            for i, a in enumerate(akuns[:50], 1):
                st = "✅" if a.get("exp", "") >= today else "❌"
                if a.get("exp", "") >= today: aktif += 1
                else: exp += 1
...
async def cb_admin_backup_do(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for b in backed:
            lines.append(f"   • {b}")
        lines.append(f"\n📅 Waktu : {_dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    except Exception as e:
...
def main():
    ...
    # ── Cleanup expired saat startup + background loop setiap 5 menit ──────
    async def _startup_cleanup(application):
        """
        Dijalankan sekali saat bot start:
        1. Hapus akun trial expired di server lokal (menangani kasus bot restart)
        2. Hapus akun trial expired di semua server remote
        3. Jadwalkan loop cleanup otomatis setiap 5 menit selama bot hidup
        """
        import asyncio as _aio

        # -- Cleanup lokal saat startup --
        try:
            result = delete_expired_accounts()
            if result["count"] > 0:
                log.info(f"[STARTUP] Cleanup: {result['count']} akun expired dihapus → {result['deleted']}")
            else:
                log.info("[STARTUP] Cleanup: tidak ada akun expired di lokal.")
        except Exception as e:
            log.warning(f"[STARTUP] Cleanup lokal gagal: {e}")

        # -- Cleanup remote saat startup --
        try:
            for srv_id, srv in load_servers().items():
                if srv.get("api_url", "").strip() and srv.get("enabled"):
                    r = api_call_remote(srv_id, "delete_expired", {})
                    if r.get("ok") and r.get("count", 0) > 0:
                        log.info(f"[STARTUP] Remote {srv_id}: {r['count']} akun expired dihapus.")
        except Exception as e:
            log.warning(f"[STARTUP] Cleanup remote gagal: {e}")

        # -- Background loop: cleanup setiap 5 menit --
        async def _cleanup_loop():
            while True:
                await _aio.sleep(300)   # setiap 5 menit
                try:
                    res = delete_expired_accounts()
                    if res["count"] > 0:
                        log.info(f"[AUTO-CLEANUP] Lokal: {res['count']} akun expired dihapus → {res['deleted']}")
                except Exception as ex:
                    log.warning(f"[AUTO-CLEANUP] Lokal gagal: {ex}")
                try:
                    for srv_id, srv in load_servers().items():
                        if srv.get("api_url", "").strip() and srv.get("enabled"):
                            r = api_call_remote(srv_id, "delete_expired", {})
                            if r.get("ok") and r.get("count", 0) > 0:
                                log.info(f"[AUTO-CLEANUP] Remote {srv_id}: {r['count']} akun expired dihapus.")
                except Exception as ex:
                    log.warning(f"[AUTO-CLEANUP] Remote gagal: {ex}")

        _aio.ensure_future(_cleanup_loop())
        log.info("[STARTUP] Background auto-cleanup setiap 5 menit aktif ✅")

    app.post_init = _startup_cleanup
