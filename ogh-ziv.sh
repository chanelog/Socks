...
setup_menu_cmd() {
    ...
    # Salin atau download script ke lokasi permanen
    if [[ "$sp" != "/usr/local/bin/ogh-ziv" ]]; then
        cp "$0" /usr/local/bin/ogh-ziv 2>/dev/null
        # Jika cp gagal (dijalankan via bash <(curl ...)), download langsung
        if [[ ! -s /usr/local/bin/ogh-ziv ]]; then
    ...
    # Symlink /usr/local/bin/menu → ogh-ziv
...
PROFEOF
    ...
    # Aktifkan alias di sesi terminal sekarang juga — tanpa perlu source manual
    # shellcheck disable=SC1090
    source ~/.bashrc 2>/dev/null || true
    alias menu='bash /usr/local/bin/ogh-ziv' 2>/dev/null || true
    alias zivpn='bash /usr/local/bin/ogh-ziv' 2>/dev/null || true
}
